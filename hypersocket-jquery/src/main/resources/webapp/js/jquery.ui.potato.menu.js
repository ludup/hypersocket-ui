...
(function($) {
	var defaults = {
		...
		menuRootClass:'potato-root-item ui-state-default',
		...
		hoverClass:'potato-menu-hover ui-state-hover',
		showDuration: 350,
		hideDuration: 100
	};
	function menu() {
		...
		var $menuItems = $menu.children(option.menuItemSelector);
		$menuItems.each(function(idx, obj) {
			$(this).addClass(option.menuRootClass);
			$(this).find(option.menuItemSelector).addClass(option.menuItemClass);
		});
		
		var $menuGroups = $menu.find(option.menuGroupSelector).addClass(option.menuGroupClass);
		
		$menuItems = $menu.find(option.menuItemSelector);
		$menuItems.hover(
			function(e) {
