...
$.fn.revertProperty = function() {
	...
	if ($(this).data('isMultipleSelect')) {
		...
		if ($(this).hasClass('switch-input')) {
			...
			log($(this).attr('id') + ' checked=' + checked + ' and has value '
					+ val);
			if (checked != val) {
				$(this).trigger('click');
			}
				...
				if (options.showButtons) {
					$(panel)
							.append(
									'<div id="'
											+ propertyDiv
											+ 'Actions" class="panel-footer tabActions"><button class="btn btn-small btn-danger" id="'
											+ propertyDiv
											+ 'Revert"><i class="fa fa-ban"></i>'
											+ getResource('text.revert')
											+ '</button><button class="btn btn-small btn-primary" id="'
											+ propertyDiv
											+ 'Apply"><i class="fa fa-check"></i>'
											+ getResource('text.apply')
											+ '</button></div>');
				}
						...
						.each(
								...
								function() {
									...
									$(contentTabs)
											.append(
													'<li><a '
															+ (first ? 'class="active clickableTab"'
																	: 'class="clickableTab"')
															+ ' href="#'
															+ tab
															+ '"><span>'
															+ getResource(this.categoryKey
																	+ '.label')
															+ '</span></a></li>');
									first = false;
									...
									$
											.each(
													...
													function() {
														...
														$('#' + tab)
																.append(
																		'<div class="propertyItem form-group" id="'
																				+ tab
																				+ '_item'
																				+ this.id
																				+ '"/>');
														...
														$(
																'#'
																		...
																		+ this.id)
																.append(
																		'<div class="propertyValue col-md-9" id="'
																				+ tab
																				+ '_value'
																				+ this.id
																				+ '"></div>');
														...
														if (obj.inputType == 'textarea') {
															$(
																	...
																	.append(
																			'<textarea '
																					+ (options.canUpdate ? ''
																							: 'disabled ')
																					+ 'class="form-control propertyInput" id="'
																					+ tab
																					+ '_input'
																					+ this.id
																					+ '" name="input'
															...
															$(
																	'#'
																			...
																			+ this.id)
																	.append(
																			'<div class="btn-group"><input id="'
																					+ tab
																					+ '_input'
																					+ this.id
																					+ '" class="propertyInput" type="hidden" name="select_value_'
																					+ this.id
																					+ '" value="'
																					+ this.value
																					+ '"><button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown"><span id="select_button_'
																					+ this.id
																					+ '">'
																					+ (obj.nameIsResourceKey ? getResource(this.value)
																							: this.value)
																					+ '</span>&nbsp;<span class="btn-icon caret"></span></button><ul id="'
																					+ inputTab
																					+ '_select'
																					+ inputId
																	...
																	if (this.value == obj.options[i].value) {
																		$(
																				'#select_button_'
																						+ inputId)
																				.text(
																						obj.nameIsResourceKey ? getResource(obj.options[i].name)
																								: obj.options[i].name);
																		$(
																				'#'
																						+ tab
																						...
																						+ this.id)
																				.append(
																						'<li><a class="selectButton_'
																								+ this.id
																								+ '" href="#" data-value="'
																								+ stripNull(obj.options[i].value)
																								+ '" data-label="'
																								+ (obj.nameIsResourceKey ? getResource(obj.options[i].name)
																										: obj.options[i].name)
																								+ '">'
																								...
																								if (option.value == inputObj.value) {
																									$(
																											'#select_button_'
																													+ inputId)
																											.text(
																													obj.nameIsResourceKey ? getResource(option.name)
																															: option.name);
																									$(
																											'#'
																													+ inputTab
															...
															$(
																	...
																	.append(
																			'<input '
																					+ (options.canUpdate ? ''
																							: 'disabled ')
																					+ 'type="password" class="form-control propertyInput" id="'
																					+ tab
																					+ '_input'
																					+ this.id
																					+ '" name="input'
															...
															$(
																	...
																	.append(
																			'<input '
																					+ (options.canUpdate ? ''
																							: 'disabled ')
																					+ 'type="checkbox" class="form-control propertyInput" id="'
																					+ tab
																					+ '_input'
																					+ this.id
																					+ '" name="input'
																					+ this.id
																					+ '" value="true"'
																					+ (stripNull(this.value) == 'true' ? ' checked'
																							: '')
																					+ '/>');
														} else if (obj.inputType == 'switch') {
															...
															$(
																	...
																	.append(
																			'<label class="switch"><input '
																					+ (options.canUpdate ? ''
																							: 'disabled ')
																					+ 'type="checkbox" class="switch-input propertyInput" id="'
																					+ tab
																					+ '_input'
																					+ this.id
																					+ '" name="input'
																					...
																					+ getResource("text.on")
																					+ '" data-off="'
																					+ getResource("text.off")
																					+ '"></span> <span class="switch-handle"></span></label>');
															...
															$(
																	...
																	.append(
																			'<input '
																					+ (options.canUpdate ? ''
																							: 'disabled ')
																					+ 'type="file" class="form-control propertyInput" id="'
																					+ tab
																					+ '_input'
																					+ this.id
																					+ '" name="input'
															...
															input
																	.change(function() {
																		...
																		reader
																				.readAsBinaryString(input[0].files[0]);
																	});
															...
															$(
																	...
																	.append(
																			'<div id="slider_'
																					+ this.id
																					+ '" class="slider"></div><input class="propertyInput" id="'
																					+ tab
																					+ '_input'
																					+ this.id
																					+ '" type="hidden" name="'
																					...
																					+ '" value="'
																					+ this.value
																					+ '"/><div class="slider-value"><span id="slider_value_'
																					+ this.id
																					+ '">'
																					+ this.value
																					+ ' '
																					+ getResource(obj.labelResourceKey)
																					+ '</span></div>');
															$(
																	'#slider_'
																			+ this.id)
															...
															var valueElement = $('#slider_value_'
																	+ this.id);
															var valueInput = $('#'
																	+ tab
																	+ '_input'
															...
															$(
																	...
																	.append(
																			'<input '
																					+ (options.canUpdate ? ''
																							: 'disabled ')
																					+ 'type="text" class="form-control propertyInput" id="'
																					+ tab
																					+ '_input'
																					+ this.id
																					+ '" size="'
...
$.fn.saveProperties = function(includeAll, callback) {
	...
	$('.propertyInput', '#' + $(this).attr('id')).each(
			function(i, obj) {
				...
				if (item.data('isMultipleSelect')
						|| item.data('isMultipleTextInput')) {
					if (includeAll || item.data('updated')) {
						items.push(new PropertyItem(item.data('resourceKey'),
					...
					if (includeAll || item.isUpdated()) {
						...
						if (item.attr('type') == "checkbox") {
							items.push(new PropertyItem(item
									.data('resourceKey'),
									item.prop("checked") ? "true" : "false"));
						} else if (item.attr('type') == "file") {
							items
									.push(new PropertyItem(item
											.data('resourceKey'), item
											.data('encoded')));
						} else {
							items.push(new PropertyItem(item
									.data('resourceKey'), item.val()));
						}
					}
				}
			});
...
$.fn.multipleSelect = function(data) {
	...
	if ($(this).data('created')) {
		...
		if ((options.selected && options.selected.length == 0)
				&& options.selectAllIfEmpty) {
			var allExcludedOptions = $('#' + $(this).attr('id')
					+ 'ExcludedSelect option');
			...
			var allIncludedOptions = $('#' + $(this).attr('id')
					+ 'IncludedSelect option');
							...
							function(idx, obj) {
								...
								select
										.append('<option '
												...
												+ obj[options.idAttr]
												+ '">'
												+ (options.nameAttrIsResourceKey ? (getResource(obj[options.nameAttr]) == undefined ? obj[options.nameAttr]
														: getResource(obj[options.nameAttr]))
			...
			$.each(data.remove, function(idx, obj) {
				if (options.selectedIsObjectList) {
					selectedOpt = $('#' + select.attr('id') + ' option[value="'
							+ obj[options.idAttr] + '"]');
					if (!selectedOpt) {
						selectedOpt = $('#' + toSelect.attr('id')
								+ ' option[value="' + obj[options.idAttr]
								+ '"]');
					}
				} else {
					selectedOpt = $('#' + select.attr('id') + ' option[value="'
		...
		if (getResourceNoDefault(options.resourceKey + '.' + $(this).attr('id')
				+ '.tooltip') != undefined) {
			$(this).append(
					'<div class="multiselectTooltip"><span id="'
							+ $(this).attr('id')
							+ 'Tooltip" class="ui-icon ui-icon-info" title="'
							+ getResource(options.resourceKey + '.'
									+ $(this).attr('id') + '.tooltip')
							+ '"></span></div>');
			$('#' + $(this).attr('id') + 'Tooltip').tooltip();
		}
		$(this).append(
				'<div class="excludedList col-md-5" id="' + $(this).attr('id')
		...
		$(this).append(
				'<div class="listButtons" id="' + $(this).attr('id')
						+ 'Buttons"/>');
		...
		$(this).append(
				'<div class="includedList col-md-5" id="' + $(this).attr('id')
		...
		$('#' + $(this).attr('id') + 'Included')
				.append(
						'<select '
								...
								+ 'multiple="multiple" id="'
								+ $(this).attr('id')
								+ 'IncludedSelect" class="formInput text form-control"/>');
		...
		$('#' + $(this).attr('id') + 'RemoveButton').click(
				function(e) {
					var selectedOpts = $('#' + toSelect.attr('id')
							+ ' option:selected');
					if (selectedOpts.length == 0) {
						e.preventDefault();
					}
		...
		$
				.each(
						...
						function(idx, obj) {
							...
							var selectItem = ((!options.selected || (options.selected && options.selected.length == 0))
									&& options.selectAllIfEmpty ? toSelect
									: select);
							selectItem
									.append('<option '
											+ 'value="'
	...
	if ($(this).data('created')) {
		...
		if (options.values) {
			$.each(options.values, function(idx, obj) {
				toSelect.append('<option ' + 'value="' + obj + '">' + obj
						+ "</option>");
			});
		}
		...
		if (getResourceNoDefault(options.resourceKey + '.' + $(this).attr('id')
				+ '.tooltip') != undefined) {
			$(this).append(
					'<div class="multisearchTooltip"><span id="'
							+ $(this).attr('id')
							+ 'Tooltip" class="ui-icon ui-icon-info" title="'
							+ getResource(options.resourceKey + '.'
									+ $(this).attr('id') + '.tooltip')
		...
		// '</label></div>');
		$(this).append(
				'<div class="excludedList" id="' + $(this).attr('id')
						+ 'Excluded"></div>');
		$('#' + $(this).attr('id') + 'Excluded')
				.append(
						'<input type="text" '
		...
		$(this).append(
				'<div class="multipleTextInputButtons" id="'
						+ $(this).attr('id') + 'Buttons"/>');
		...
		// '</label></div>');
		$(this).append(
				'<div class="includedList" id="' + $(this).attr('id')
						+ 'Included"></div>');
		$('#' + $(this).attr('id') + 'Included').append(
				'<select ' + (!options.disabled ? '' : 'disabled ')
						+ 'multiple="multiple" id="' + $(this).attr('id')
						+ 'IncludedSelect" class="formInput text form-control'
						+ (options.isPropertyInput ? ' propertyInput' : '')
						+ '"/>');
		...
		$('#' + $(this).attr('id') + 'AddButton').click(
				function(e) {
					...
					toSelect.append('<option ' + 'value="' + selectedText
							+ '">' + selectedText + "</option>");
					select.val('');
...
$.fn.resourcePage = function(params) {
	...
	$(this)
			.append(
					'<div id="'
							+ divName
							+ 'Panel" class="panel-body"><div class="row"><div class="col-lg-6"><div class="numRecords">'
							...
							+ divName + 'Table' + '"><thead><tr id="' + divName
							+ 'TableHeader"></tr></thead></table></div>');

	$(this).append(
			'<div id="' + divName
					+ 'Actions" class="tabActions panel-footer"/>');
	...
	getJSON(
			...
			function(data) {
				...
				// + ' row-' + act.resourceKey +'" title="' +
				// getResource("text." + act.resourceKey) + '"></span>';
				//							
				// $(document).off('click', '#' + divName + 'Actions' + id + '
				// .row-' + act.resourceKey);
				//								
				// $(document).on('click', '#' + divName + 'Actions' + id + '
				// .row-' + act.resourceKey, function () {
				// var curRow = $('#' + divName +
				// 'Table').dataTable().fnGetPosition($(this).closest("tr").get(0));
				// var resource = $('#' + divName +
				// 'Table').dataTable().fnGetData(curRow);
				// act.action(resource);
				// });
				// }
				// });
				...
				// renderedActions += '<span class="ui-icon ui-icon-wrench
				// row-edit" title="' + getResource("text.edit") + '"></span>';
				//					
				// $(document).off('click', '#' + divName + 'Actions' + id + '
				// .row-edit');
				//						
				...
				// renderedActions += '<span class="ui-icon ui-icon-trash
				// row-delete" title="' + getResource("text.delete") +
				// '"></span>';
				//					
				// $(document).off('click', '#' + divName + 'Actions' + id + '
				// .row-delete');
				...
				// var resource = $('#' + divName +
				// 'Table').dataTable().fnGetData(row);
				// confirmBox( { title: getResource(options.resourceKey +
				// ".delete.title"),
				// message: getResource(options.resourceKey +
				// ".delete.desc").format(resource.name),
				// callback: function() {
				// deleteJSON(options.resourceUrl + "/" + id, null,
				// function(data) {
				// if(data.success) {
				...
				// } else {
				// msgBox({ title: getResource("text.error"), message:
				// data.message });
				// }
				// });
				// }
				...
				// }
				//					
				// return '<div id="' + divName + 'Actions' + id + '"
				// class="tableActions">' + renderedActions + '</div>';
				// };

				$('#' + divName + 'TableHeader').append(
						'<th localize="text.actions"></th>');

				$('#' + divName + 'Table').append(
						'<tbody id="' + divName + 'Body" ></tbody');
								...
								function(idx, obj) {
									...
									$
											.each(
													...
													function(idx2, col) {
														row += '<td>'
																+ (col.isResourceKey ? getResource(options.resourceKey
																		+ "."
																		+ obj[col.name]
																		+ '.label')
																		: obj[col.name])
										...
										$
												.each(
														...
														function(x, act) {
															if (act.enabled) {
																row += '<a class="btn '
																		+ (act.buttonClass ? act.buttonClass
																				: 'btn-success')
																		+ '" href="#" resource-value="'
																		+ obj[options.idField]
																		+ '"><i class="fa '
									...
									if (options.canUpdate) {
										row += '<a class="btn btn-info editAction" href="#" resource-value="'
												+ obj[options.idField]
												+ '"><i class="fa fa-edit"></i></a>';
									}

									if (options.canDelete) {
										row += '<a class="btn btn-danger deleteAction" href="#" resource-value="'
												+ obj[options.idField]
				...
				$('#' + divName + 'Panel')
						.append(
								'<div class="tableInfo">Showing 1 to 10 of 32 entries</div>');
				$('#' + divName + 'Panel')
						.append(
								'<div class="tablePages paging_bootstrap"><ul class="pagination"><li class="prev disabled"><a href="#">← Previous</a></li><li class="active"><a href="#">1</a></li><li><a href="#">2</a></li><li><a href="#">3</a></li><li><a href="#">4</a></li><li class="next"><a href="#">Next → </a></li></ul></div>');
				if (options.canCreate) {
					$('#' + divName + 'Actions').append(
							'<button class="btn btn-primary" id="' + divName
...
$.fn.ajaxResourcePage = function(params) {
	...
	$(this)
			.append(
					'<div class="panel panel-default"><div class="panel-heading"><h2><i class="fa '
							...
							+ '</span></h2></div><div id="'
							+ divName
							+ 'Panel" class="panel-body"><table class="table table-striped" id="'
							+ divName
							+ 'Table'
							+ '"><thead><tr id="'
							+ divName
							+ 'TableHeader"></tr></thead></table></div><div id="'
							+ divName
	...
	$.each(options.fields, function(idx, obj) {
		$('#' + divName + 'TableHeader').append(
				'<th>'
						+ getResource(options.resourceKey + "." + obj.name
								+ '.label') + '</th>');
		columns.push({
			"mData" : obj.name
		});
		if (obj.isResourceKey) {
			columnsDefs.push({
				...
				"mRender" : function(data, type, full) {
					return getResource(options.resourceKey + "." + data
							+ '.label');
				}
			});
		}
		...
		if (options.additionalActions) {
			$.each(options.additionalActions, function(x, act) {
				if (act.enabled) {
					...
					renderedActions += '<a class="btn '
							+ (act.buttonClass ? act.buttonClass
									: 'btn-success') + ' row-'
							+ act.resourceKey + '" href="#"><i class="fa '
							+ act.iconClass + '"></i></a>';

					$(document).off(
							'click',
							'#' + divName + 'Actions' + id + ' .row-'
									+ act.resourceKey);
					...
					$(document).on(
							...
							'#' + divName + 'Actions' + id + ' .row-'
									+ act.resourceKey,
							function() {
								var curRow = $('#' + divName + 'Table')
										.dataTable().fnGetPosition(
												$(this).closest("tr").get(0));
								var resource = $('#' + divName + 'Table')
										.dataTable().fnGetData(curRow);
								act.action(resource);
							});
				}
			...
			$(document).on(
					...
					function() {
						...
						var resource = $('#' + divName + 'Table').dataTable()
								.fnGetData(curRow);
						$('div[dialog-for="' + divName + '"]').resourceDialog(
								'edit', {
									row : curRow,
									resource : resource
								});
			...
			$(document)
					.on(
							...
							function() {
								...
								var row = $(this).closest("tr").get(0);
								var resource = $('#' + divName + 'Table')
										.dataTable().fnGetData(row);
								confirmBox({
									title : getResource(options.resourceKey
											+ ".delete.title"),
									message : getResource(
											options.resourceKey
													+ ".delete.desc").format(
...
$.fn.resourceDialog = function(params, params2) {
	...
	if (params === 'create') {
		...
		$(this).dialog(
				...
				[ {
					...
					click : function() {
						...
						postJSON(dialogOptions.resourceUrl, resource, function(
								data) {
							if (data.success) {
								log("Resource object created");
								dialog.resourceDialog('close');
								if (dialogOptions.hasResourceTable) {
									$('#' + dialogOptions.divName + 'Table')
											.dataTable().fnAddData(
													data.resource);
								}
								if (dialogOptions.resourceCreated) {
									dialogOptions
											.resourceCreated(data.resource);
								}
								showInformation(true, data.message);
							} else {
								log("Resource object creation failed "
										+ data.message);
		...
		$(this).dialog(
				'option',
				'title',
				getResource($(this).data('options').resourceKey
						+ '.update.title'));
		$(this).dialog('option', 'position', {
			my : "center",
			at : "center",
					...
					.each(
							function(i, obj) {
								...
								if (label.attr('for') != undefined) {
									if (getTooltip(options.resourceKey, label
											.attr('for')) != undefined) {
										$(obj)
												.prepend(
...
function processLogon(data, message) {
	...
	$('#copyright')
			.append(
					"<p>"
							+ getResource("label.version")
							+ " "
							+ data.version
							+ "</p><p>&copy; 2013-14 Hypersocket Limited. All rights reserved.</p>");
	...
	if (!data.success) {
			...
			$('#logonForm')
					.append(
							'<h2 class="form-signin-heading">'
									+ (data.lastErrorIsResourceKey ? getResource(data['errorMsg'])
											: data['errorMsg']) + '</h2>');
		} else if (message) {
			$('#logonForm').append(
		...
		$.each(data['formTemplate']['inputFields'], function() {
			...
			if (this.type == 'select') {
				$('#logonForm').append(
						'<select name="' + this.resourceKey + '" id="'
								+ this.resourceKey + '"/>');
				...
				$.each(this.options, function() {
					...
					option += '>'
							+ (this.isNameResourceKey ? getResource(this.name)
									: this.name) + '</option>';
				...
				$('#logonForm').append(
						'<input class="form-control" type="' + this.type
								+ '" name="' + this.resourceKey
								+ '" placeholder="'
								+ getResource(this.resourceKey + ".label")
								+ '" id="' + this.resourceKey + '" value="'
								+ this.defaultValue + '"/>');
			}
		...
		$('#logonForm')
				.append(
						'<button id="logonButton" class="btn btn-lg btn-primary btn-block" type="submit">'
								+ getResource("text.logon")
								+ '<i style="float: right; padding: 4px 10px 0px 0px" class="fa fa-sign-in"></i></button>');
		$('#logonButton').click(
				function(evt) {
					...
					$.each(data['formTemplate']['inputFields'], function() {
						credentials = credentials
								+ '&'
								+ encodeURIComponent(this.resourceKey)
								+ '='
								+ encodeURIComponent($('#' + this.resourceKey)
										.val());
					});
		...
		if (data['bannerMsg']) {
			$(contentDiv).append(
					'<div class="col-md-3"></div><div id="logonBanner" class="col-md-6"><p>'
							+ data['bannerMsg']
							+ '</p></div><div class="col-md-3"></div>');
		}
				...
				if (showLocales) {
					...
					$('#lang').change(
							function() {
								...
								getJSON('switchLanguage/' + this.value, null,
										function() {
											document.location.reload();
										});
							});
...
function loadRealms(realms) {
	...
	$('#currentRealm option[value="' + currentRealm.id + '"]').attr('selected',
			true);
	...
	$('#currentRealm').on(
			...
			function(evt) {
				getJSON('switchRealm/'
						+ $('#currentRealm option:selected').val(), null,
						function(data) {
							if (!data.success) {
								...
								$
										.each(
												...
												function(idx, obj) {
													$
															.pnotify({
																title : obj.titleIsResourceKey ? getResource(obj.title)
																		: obj.title,
																text : obj.textIsResourceKey ? getResource(obj.text)
																		: obj.text
															});
												});
...
function loadMenu(menu) {
	...
	$('#mainContent').load('content/' + menu.resourceName + '.html',
			function() {
				loadWait();
			});
