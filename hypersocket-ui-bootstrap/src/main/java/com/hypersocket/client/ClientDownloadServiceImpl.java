...
public class ClientDownloadServiceImpl implements ClientDownloadService {
	...
	File downloadsDir = new File(System.getProperty(
			"hypersocket.conf", "conf"), "clients");
	...
	private void postConstruct() {
		...
		try {
			in = new FileInputStream(new File(downloadsDir, "downloads.properties"));
			props.load(in);
		...
		if(downloads.size() > 0) {
			...
			menuService.registerMenu(new MenuRegistration(RESOURCE_BUNDLE, "clientDownloads",
					"fa-download", "clientDownloads", 999,
